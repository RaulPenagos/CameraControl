...
class MyLikelihood(GenericLikelihoodModel):
   ...
   def loglike(self, params):
      ...
      # for i in range(0, self.n):
      #    new_measure2 = measurements2[i]
      #    new_measure3 = measurements3[i]   # x y z medidas
      #    cPoint_p = self.cPoints[i]
      #    rPoint = self.rPoints[i]
      #    cPoint = cPoint_p[0], cPoint_p[1], cPoint_p[4], cPoint_p[5]  # J1, J2, X, Y
      #    chi2 += np.linalg.norm(new_measure2-cPoint)**2 +  np.linalg.norm(new_measure3-rPoint)**2
...
class Calibration():
   ...
   def calibrate(self):
      ...
      print(results.summary(xname = ['x','y','z', 'phi', 'psi','psi2']))
...
def main():
   ...
   # Generate the camera  
   camera = Camera(x = 5.0, y = 0.4, z = 2.0, psi = -1.5*np.pi/180, theta =  np.pi/180, phi = -2*np.pi/180, cx = -0.5, cy = -0.5, focaldistance = 10, sigmaCamera = 0.001)
